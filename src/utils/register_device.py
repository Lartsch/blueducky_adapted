...
def agent_loop(target_path):
    ...
    logger.debug("'NoInputNoOutput' pairing-agent is running")
    loop.run()
...
<record>
    ...
    logger.debug("calling RegisterProfile")
    manager.RegisterProfile(profile, hid_uuid, opts)
    loop = GLib.MainLoop()
    try:
        logger.debug("running dbus loop")
        loop.run()
    except KeyboardInterrupt:
        logger.debug("calling UnregisterProfile")
